...
def lambda_handler(event, context):
    ...
    Event examples:
      {"table": "projects"}
      {"table": "timesheets"}
      {"tables": ["projects","sale_orders","partners","timesheets"]}
      {"all": true}
    """

    print("EVENT:", json.dumps(event))  # helpful for debugging

    # Handle API Gateway / Function URL payload
    if "body" in event and event["body"]:
        try:
            body = json.loads(event["body"])
            if isinstance(body, dict):
                event.update(body)  # merge parsed body into event
    ...
    for table in tables:
        ...
        data = odoo_search_read(model, fields)  # no domain, always full
